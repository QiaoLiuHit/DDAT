...
class LSOTBTIR(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=default_image_loader, min_length=0, max_target_area=1):
        ...
        super().__init__('lsotbtir', root, image_loader)
    ...
    def _get_frame(self, sequence, frame_id):
        set_name = 'TIR_training_{:03d}'.format(sequence['set_id'])
        vid_name = sequence['vid_name']
        frame_number = frame_id + sequence['start_frame'] + 1

        frame_path = os.path.join(self.root, 'TrainingData', set_name, vid_name,
                                  '{:08d}.jpg'.format(frame_number))
    ...
    def _process_anno(self, root):
        ...
        base_vid_anno_path = os.path.join(root, 'Annotations')
        ...
        for set in sorted(os.listdir(base_vid_anno_path)):
            ...
            for vid in sorted(os.listdir(os.path.join(base_vid_anno_path, set))):
                ...
                vid_name = vid
                anno_files = sorted(os.listdir(os.path.join(base_vid_anno_path, set, vid_name)))

                frame1_anno = ET.parse(os.path.join(base_vid_anno_path, set, vid_name, anno_files[0]))
                image_size = [int(frame1_anno.find('size/width').text), int(frame1_anno.find('size/height').text)]

                objects = [ET.ElementTree(file=os.path.join(base_vid_anno_path, set, vid_name, f)).findall('object')
                           for f in anno_files]
                ...
                for tracklet_id, tracklet_start in tracklets.items():
                    ...
                    new_sequence = {'set_id': set_id, 'vid_name': vid_name, 'class_name': class_name_id,
