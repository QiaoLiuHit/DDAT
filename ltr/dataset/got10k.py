...
class Got10k(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        if split is not None:
            ...
            seq_ids = pandas.read_csv(file_path, header=None, dtype=np.int64).values.tolist()#squeeze=True,
        elif seq_ids is None:
            seq_ids = list(range(0, len(self.sequence_list)))

        self.sequence_list = [self.sequence_list[i[0]] for i in seq_ids]
    ...
    def _read_meta(self, seq_path):
        try:
            with open(os.path.join(seq_path, 'meta_info.txt')) as f:
