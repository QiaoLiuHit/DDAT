...
class ResNet(Backbone):
    ...
    def __init__(self, block, layers, output_layers, num_classes=1000, inplanes=64, dilation_factor=1, frozen_layers=()):
        ...
        self.layer1 = self._make_layer(block, inplanes, layers[0], stride=1, dilation=max(dilation_factor//8, 1))
    ...
    def forward(self, x, output_layers=None):
        ...
        x = self.conv1(x) #tir [2, 3, 127, 127] -> [2, 64, 64, 64]  [2, 3, 511, 511] -> [2, 64, 256, 256]
        x = self.bn1(x)
        x = self.relu(x)
        ...
        x = self.layer1(x)#tir [2, 64, 64, 64]->[2, 64, 32, 32]     [2, 64, 256, 256]->[2, 256, 128, 128]

        if self._add_output_and_check('layer1', x, outputs, output_layers):
            return outputs

        x = self.layer2(x)#tir [2, 64, 32, 32]->[2, 512, 16, 16] [2, 256, 128, 128]->[2, 512, 64, 64]

        if self._add_output_and_check('layer2', x, outputs, output_layers):
            return outputs

        x = self.layer3(x)#tir [2, 512, 16, 16]->[2, 1024, 16, 16]   [2, 512, 64, 64]->[2, 1024, 64, 64]
