...
class BackboneBase(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        for name, x in xs.items():
            ...
            out[name] = NestedTensor(x, mask)#将通过resnet后特征和mask融合一起   x[2, 1024, 16/32, 16/32] mask[2,16/32,16/32] 1024是通道数，每层加的掩码都一样，所以mask只要batchsize、长宽一样即可。
        return out
...
# here can out multi-layers feature using output_layers=['layer3', 'layer2', 'layer1' ]
